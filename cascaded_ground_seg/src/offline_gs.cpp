...
private:
	...
	std::string output_gtpoints_name;
...
void CascasedGroundSeg::pub_score(std::string mode, double measure) {
    static int                 SCALE = 5;
...
void CascasedGroundSeg::FillIndexMap(const pcl::PointCloud<PointType>::ConstPtr &in_cloud_msg)
{
...
void CascasedGroundSeg::ColumnSegment(int i, const pcl::PointCloud<PointType>::ConstPtr &in_cloud_msg, std::vector<int> &v_ring)
{
...
void CascasedGroundSeg::FillRegionIndex(pcl::PointCloud<PointType>::Ptr &remaining_ground_cloud)
{
...
void CascasedGroundSeg::PlaneSeg(int q, int s, pcl::PointCloud<PointType>::Ptr &region_cloud, pcl::PointCloud<PointType> &out_ground_points, pcl::PointCloud<PointType>::Ptr &remaining_vertical, pcl::ModelCoefficients::Ptr &prev_coefficients)
{
...
void CascasedGroundSeg::SectionPlaneSegment(int i, pcl::PointCloud<PointType>::Ptr &remaining_ground, pcl::PointCloud<PointType> &out_ground_points, pcl::PointCloud<PointType>::Ptr &remaining_vertical)
{
...
void CascasedGroundSeg::SegmentGround(const pcl::PointCloud<PointType>::ConstPtr &in_cloud_msg,
			pcl::PointCloud<PointType> &out_groundless_points,
			pcl::PointCloud<PointType> &out_ground_points)
{
...
void CascasedGroundSeg::VelodyneCallback(const pcl::PointCloud<PointType>::ConstPtr &in_cloud_msg)
{
	...
	elap_time_ = boost::chrono::duration_cast<boost::chrono::nanoseconds>(t2_-t1_);
     ...
     // Estimation
        ...
        cout << "\033[1;32m" << n << "th, " << " takes : " << take_time / 1e9 << " | " << (*in_cloud_msg).size() << " -> "
             << ground_points.size()
             << "\033[0m" << endl;
		...
		if(save_csv){
        	...
        	ground_output << n << "," << take_time << "," << precision << "," << recall << "," << precision_naive << "," << recall_naive;
...
int main(int argc, char **argv)
{
	...
	ros::NodeHandle nh;
	nh.param<std::string>("data_path", data_path, "/");
    ...
    for (int n = 0; n < N; ++n) {
		...
		node.VelodyneCallback(laserCloudInPtr);
